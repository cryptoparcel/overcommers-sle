...
class EventForm(FlaskForm):
    ...
    start_time = StringField("Start time (e.g. 7:00 PM)", validators=[Optional(), Length(max=30)])
    end_time = StringField("End time (optional)", validators=[Optional(), Length(max=30)])

    location_name = StringField("Location name (e.g. Overcomers House A)", validators=[Optional(), Length(max=180)])
    location_address = StringField("Address (optional)", validators=[Optional(), Length(max=255)])
    ...
    category = SelectField(
        ...
        choices=[
            ...
            ("milestone", "Milestone / Graduation"),
            ("support", "Support Group"),
        ],
        default="general",
    )

    summary = TextAreaField("Short summary (1-2 sentences)", validators=[Optional(), Length(max=320)])
